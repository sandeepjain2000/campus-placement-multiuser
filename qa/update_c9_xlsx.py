...
def get_test_status(results):
    ...
    def traverse(node):
        if isinstance(node, dict):
            if 'title' in node and 'C9-' in node['title'] and 'tests' in node:
                match = re.search(r'(C9-[A-Z]+-\d+)', node['title'])
                if match:
...
def update_excel():
    json_file = 'c9_results.json'
    excel_file = 'C-9-Transactional-Lifecycle-Add-Edit-Delete-View-Cancel.xlsx'
