...
def get_test_status(results):
    ...
    def traverse(node):
        if isinstance(node, dict):
            if 'title' in node and 'C2-' in node['title'] and 'tests' in node:
                match = re.search(r'(C2-[A-Z]+-\d+)', node['title'])
                if match:
...
def update_excel():
    json_file = 'c2_results.json'
    excel_file = 'C-2-Tabular-Screens-CSV-Export.xlsx'
