...
def get_test_status(results):
    ...
    def traverse(node):
        if isinstance(node, dict):
            if 'title' in node and 'C10-' in node['title'] and 'tests' in node:
                match = re.search(r'(C10-[A-Z]+-\d+)', node['title'])
                if match:
...
def update_excel():
    json_file = 'c10_results.json'
    excel_file = 'C-10-Profile-Editability-and-Immutable-Identity-Fields.xlsx'
