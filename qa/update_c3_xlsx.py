...
def get_test_status(results):
    ...
    def traverse(node):
        if isinstance(node, dict):
            if 'title' in node and 'C3-' in node['title'] and 'tests' in node:
                match = re.search(r'(C3-[A-Z]+-\d+)', node['title'])
                if match:
...
def update_excel():
    json_file = 'c3_results.json'
    excel_file = 'C-3-Transactional-Flow-and-Cross-Role-Visibility.xlsx'
