...
export default function CollegeSponsorshipsPage() {
  ...
  const {
    ...
    error,
    mutate
  } = useSWR('/api/college/sponsorships', fetcher);
  ...
  const {
    ...
    getSearchText: p => [p.companyName, p.tierName, p.method, p.status, p.billingLegalName, p.reference].filter(Boolean).join(' '),
  ...
  const downloadGuide = () => {
    const lines = sponsorshipLevels.flatMap(level => [`${level.category}`, `${level.description || ''}`, ...level.tiers.map(tier => `- ${tier.name}: ${tier.price} (${(tier.benefits || []).join('; ')})`), '']);
  ...
  const sendTaxReceipt = async paymentId => {
    ...
    try {
      ...
      addToast(`Receipt ${json.receiptNumber || ''} emailed to ${json.toEmail || 'employer'}.`, 'success');
      await mutate();
    } catch {
  ...
  const visibleLevels = useMemo(() => sponsorshipLevels.filter(s => activeTab === 'All Categories' || s.category === activeTab), [activeTab, sponsorshipLevels]);
  return <div className="animate-fadeIn" style={{
    paddingBottom: '3rem'
      ...
      padding: '1.25rem 1.5rem'
        ...
        {paymentsTotalCount === 0 ? <p className="text-sm text-secondary" style={{
          margin: 0
        }}>No payments recorded yet.</p> : <>
            <DataTableToolbar search={search} onSearchChange={setSearch} searchPlaceholder="Search employer, tier, or status…" sort={sort} onSortChange={setSort} sortOptions={COMMON_SORT_OPTIONS} filteredCount={filteredCount} totalCount={paymentsTotalCount} hasActiveFilters={hasActiveFilters} onClear={clearFilters} style={{
            marginBottom: '1rem'
          }} />
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead style={{
                    width: 1
                  }}>Tax receipt</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {displayPayments.length === 0 ? <TableRow>
                    <TableCell colSpan={8} className="text-center text-secondary">
                      No payments match your search.
                    </TableCell>
                  </TableRow> : null}
                {displayPayments.map(p => <TableRow key={p.id}>
                    <TableCell className="text-sm">{p.createdAt ? new Date(p.createdAt).toLocaleString() : '—'}</TableCell>
                    <TableCell>
                      <div>
                        <CompanyNameLink name={p.companyName} website={p.companyWebsite} />
                      </div>
                      {p.billingLegalName || p.billingPan || p.billingGstNumber ? <div className="text-xs text-tertiary" style={{
                      marginTop: '0.25rem',
                      lineHeight: 1.4
                    }}>
                    ...
                    <TableCell className="text-sm">{p.tierName} <span className="text-tertiary">({p.category})</span></TableCell>
                    <TableCell>{p.method}</TableCell>
                    <TableCell className="text-sm">{formatPaymentStatus(p.status, p.method)}</TableCell>
                    ...
                    whiteSpace: 'nowrap'
                      ...
                      {p.receiptSent ? <span className="text-xs text-secondary" title={p.receiptNumber || ''}>
                          Sent
                          {p.receiptSentAt ? ` · ${new Date(p.receiptSentAt).toLocaleDateString('en-IN')}` : ''}
                        </span> : <Button type="button" disabled={sendingReceiptFor === p.id} title="Email donation/sponsorship receipt to employer" onClick={() => void sendTaxReceipt(p.id)} variant="outline" size="sm">
                          <FileCheck2 size={14} style={{
                        marginRight: 4
                      }} aria-hidden />
              ...
              {level.tiers.map((tier, ti) => <div key={ti} style={{
                ...
                padding: '1.5rem',
                  ...
                  marginBottom: '1rem'
                    ...
                    {tier.label && <StatusBadge tone="indigo">{tier.label}</StatusBadge>}
                  </div>
                  <ul style={{
                  padding: 0,
                  listStyle: 'none',
      ...
      {!error && sponsorshipLevels.length === 0 && <Card style={{
      marginTop: '1rem'
    }}><CardContent>
