...
export default function StartupFundingMobile() {
  ...
  const downloadGuide = () => {
    const lines = fundingLevels.flatMap(level => [`${level.category}`, `${level.description || ''}`, ...level.tiers.map(tier => `- ${tier.name}: ${tier.price} (${(tier.benefits || []).join('; ')})`), '']);
  ...
  const visibleLevels = useMemo(() => fundingLevels.filter(s => activeCat === 'All Categories' || s.category === activeCat), [activeCat, fundingLevels]);
  return <>
      <MobileHeader title="Startup seed funding" action={<Button onClick={downloadGuide} style={{
      padding: '0.4rem',
      ...
      padding: '1rem 1rem 5rem 1rem'
        ...
        lineHeight: 1.45
          ...
          alignItems: 'flex-start'
            ...
            color: 'var(--text-secondary)'
              ...
              {disclaimer || 'Informational catalog only. Seed investments are negotiated and executed offline — not through PlacementHub.'}
            </p>
          </div>
        </div>
        ...
        textAlign: 'center'
            ...
            margin: 0
          }}>Failed to load startup funding programs.</p>
