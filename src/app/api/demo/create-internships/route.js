...
async function __platform_POST(request) {
  ...
  try {
    ...
    const payload = await createDemoJobs({ tenantId, count, jobType: 'internship' });
    return NextResponse.json({ ok: true, timestamp: new Date().toISOString(), ...payload });
  } catch (e) {
    console.error('POST /api/demo/create-internships', e);
    return NextResponse.json({ ok: false, error: e.message || 'Failed to create internships' }, { status: 500 });
  }
}
...
}, { context: 'api_demo_create_internships' });
