...
async function __platform_POST(request) {
  ...
  try {
    ...
    const payload = await applyDemoStudentToJob({ tenantId, jobId, jobType: 'internship' });
    const status = payload.ok === false ? 400 : 200;
    return NextResponse.json({ timestamp: new Date().toISOString(), ...payload }, { status });
  } catch (e) {
    console.error('POST /api/demo/apply-to-internship', e);
...
}, { context: 'api_demo_apply_to_internship' });
