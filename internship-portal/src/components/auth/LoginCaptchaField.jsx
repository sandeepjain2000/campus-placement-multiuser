    ...
    return (
      ...
      >
        ...
        <div className="ip-gemini-security__row">
          <div className="ip-gemini-security__badge">{equationBadge}</div>
          <input
            ref={inputRef}
            id={inputId}
            ...
            disabled={disabled || loading || !(tokenRef.current || token)}
            aria-invalid={verifyState === 'invalid'}
            className={cn(
              'ip-gemini-security__answer',
              verifyState === 'invalid' && 'border-red-500',
              (disabled || loading || !(tokenRef.current || token)) && 'cursor-not-allowed opacity-60',
            )}
          />
        </div>
        {verifyState === 'invalid' ? (
