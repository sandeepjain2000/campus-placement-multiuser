/**
 * Populate Internship Portal with Gmail/+-alias cast for table density.
 * Does NOT delete or overwrite the three “real” showcase logins if present.
 *
 * Bases (from ops):
 *   candidate: lawsonlclintern@gmail.com  → aliases +1 +2 +3
 *   employer:  shreekar.nyayapathi23@vit.edu → aliases +2 +3
 *   superadmin: placementhubsupport@gmail.com (untouched)
 *
 * Password for all demo cast accounts: Admin@123
 *
 * Usage:
...
const SA_EMAIL = 'placementhubsupport@gmail.com';
...
async function main() {
  ...
  console.log('Loading Gmail/+ demo accounts (password Admin@123 for new accounts)…');
  ...
  // Touch base candidate if present (add profile completeness fields only if missing profile)
  const baseCandUser = await client.query(`SELECT id FROM ip_users WHERE lower(email) = lower($1)`, [CAND_BASE]);
  if (baseCandUser.rows[0]) {
    const cid = await ensureCandidate(client, baseCandUser.rows[0].id, CAND_BASE, 'Lawson Candidate');
    candIds[CAND_BASE] = cid;
    candUserIds[CAND_BASE] = baseCandUser.rows[0].id;
    console.log('linked base candidate', CAND_BASE);
  }
  ...
  const baseEmpUser = await client.query(`SELECT id FROM ip_users WHERE lower(email) = lower($1)`, [EMP_BASE]);
  if (baseEmpUser.rows[0]) {
    empUserIds[EMP_BASE] = baseEmpUser.rows[0].id;
    empIds[EMP_BASE] = await ensureEmployer(
      client,
      baseEmpUser.rows[0].id,
      EMP_BASE,
      'VIT Showcase Employer',
      'approved',
    );
    console.log('linked/approved base employer', EMP_BASE);
  }
  ...
  const postEmpEmail = empIds[EMP_BASE] ? EMP_BASE : 'shreekar.nyayapathi23+2@vit.edu';
  ...
  const reqEx = await client.query(
    ...
    ['shreekar.nyayapathi23+3@vit.edu'],
  );
  if (!reqEx.rows[0]) {
    await client.query(
  ...
  console.log('\nDone. Cast logins (password Admin@123):');
