...
def get_test_status(results):
    ...
    def traverse(node):
        if isinstance(node, dict):
            if 'title' in node and 'C8-' in node['title'] and 'tests' in node:
                match = re.search(r'(C8-[A-Z]+-\d+)', node['title'])
                if match:
...
def update_excel():
    json_file = 'c8_results.json'
    excel_file = 'C-8-Alerts-Notifications-Trigger-Visibility-and-Control.xlsx'
