...
def get_test_status(results):
    ...
    def traverse(node):
        if isinstance(node, dict):
            if 'title' in node and 'C5-' in node['title'] and 'tests' in node:
                match = re.search(r'(C5-[A-Z]+-\d+)', node['title'])
                if match:
...
def update_excel():
    json_file = 'c5_results.json'
    excel_file = 'C-5-Tabular-Screens-CSV-Upload-Validation.xlsx'
